...
(define (merge-actions actions merged)
  (for/fold ((merged merged)) ((action (dict-ref actions 'actions actions)))
    (let-values (((name a.pre a.post a.touch a.enc) (apply values action)))
      (let ((old (dict-ref merged name #f)))
        (if (not old) (dict-set merged name `(,a.pre ,a.post ,a.touch ,a.enc))
            (let-values (((pre post touch enc) (apply values old)))
              ...
              (let ((touch (if (member post (list pre a.post #f))
                ...
                (dict-set merged name `(,pre ,a.post ,touch ,enc)))))))))
     ...
     ,@(let ((fs (ae-file->value (path-replace-suffix path ".fs"))))
         (for/fold ((actions '())) ((src (dict-ref fs 'src '())))
           (let* ((name     (dict-ref src 'file-name))
                  ...
                  (encoding (and post (dict-ref current 'encoding 'none))))
             (dict-set actions name `(,pre ,post () ,encoding))))))))
